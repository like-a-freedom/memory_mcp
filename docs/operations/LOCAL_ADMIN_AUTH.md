...
| Layer | Evidence |
|---|---|
| Service + durable registry (in-memory SurrealDB) | `cargo test -p memory_mcp --lib --features control-plane,test-fixtures --locked` — 1746 passed, 1 ignored |
| Service security experiments against the **real** store | `... --lib ... service::local_admin` — 33 passed (of which 13 are the plan §5 experiments; see §13.3 for the per-case mapping) |
| HTTP surface (real router, real pre-auth cookie, CSRF, Origin, session, SQL transactions, migrated in-memory registry) | `cargo test -p memory_mcp --features control-plane,test-fixtures --locked --test http_local_admin` — 37 passed |
| OIDC regression | `... --test http_control_plane` — 10 passed |
| CLI contract (clap surface + real subprocess against file-backed RocksDB) | `... --test local_admin_cli` — 6 passed |
| Durable query-shape regression | `... --test registry_query_shape` — 4 passed |
| Provisioning crash recovery | `... --test http_crash_recovery` — 10 passed |
...
| UI crate | `cargo test -p control-plane-ui --locked` — 51 passed; `cargo check -p control-plane-ui --locked` clean |
...
`docs/operations/LOCAL_ADMIN.md` predates this implementation and does **not**
match the shipped code. Concretely, it documents `memory_mcp admin create ops.one`
...
`SURREALDB_CONTROL_*` and `SURREALDB_TENANT_*`). Treat this file as authoritative
and that file as stale; reconciling or deleting it is outside the scope of this
...
| `cargo test -p memory_mcp --lib --features control-plane,test-fixtures --locked service::local_admin` | ✅ executed, 33 passed |
