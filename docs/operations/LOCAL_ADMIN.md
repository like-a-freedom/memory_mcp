...
| Rate-window boundary across replicas and after a restart | ✅ `local_admin_rate_window_survives_a_store_reconnect` spends the whole challenge budget on one connection, drops it mid-window, and shows a fresh connection still sees the spent budget (`Limited`, `retry_after_seconds > 0`); the fixed window and its saturation stay covered by `exp9`/`exp9b` |
...
| KDF completes, pause before session insert; recover on a second handle | `exp18_kdf_finishing_during_recovery_never_yields_an_old_generation_session` (the real KDF run is the pause window; both commit orders share one invariant), `reset_changes_the_password_and_fences_old_sessions` |
| Reset hash prepared, newer recovery commits | `exp19_a_reset_racing_a_newer_recovery_never_outlives_it` (real concurrency, both commit orders), `exp3_recovery_invalidates_old_credentials` (generation) |
| Resolve/touch and recovery/logout, both commit orders | `exp20_resolve_touch_racing_recovery_never_resurrects_the_session` (resolve vs recovery race; both orders assert the same invariant), `exp4_revoke_after_login` |
| Client mutation paused after middleware, logout commits | `client_mutation_follows_the_session_fence` |
| Two rotations | `exp21_concurrent_reauthentications_have_one_winner` (exactly one winner under real concurrency), `exp6_reauth_rotates_session` |
...
| Plan T10 “require operator-supplied … seven limits” | `docker-compose.yml` pre-fills the seven plan-limit variables with `PlanLimits::default()` values (all `${VAR:-default}`, operator-overridable) so a local deployment starts from the file alone. The server contract is unchanged: all seven must parse, a stored plan whose limits differ fails startup, and the values are never browser input. Verdict: keep the Compose defaults as the documented local-profile convenience and this register entry as the record of the divergence. |
