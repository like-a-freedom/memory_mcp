...
| 2 | 🔶 stage 2 in progress | ADR-0027 stage 1 landed: 6 `Ok(vec![])`/`Ok(0)` stub defaults removed from `DbClient`, MockDbClient extended, 19 test doubles made explicit. Stage 2 (SQL moves into narrow stores + mock shrink): **reembed slice landed** — `ReembedStoreClient` (`storage/reembed_store.rs`) owns `count/select_facts_needing_reembed`; SQL + Surreal impl + trait methods + 19 test-double stubs removed from `DbClient`/`MockDbClient`; reembed.rs wired through `reembed_store()`; 3 store unit tests (missing-table, stale-signature, cursor/limit). Remaining: context → app → fact → episode stores |
