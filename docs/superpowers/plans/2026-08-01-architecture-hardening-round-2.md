...
| 2 | 🔶 stage 2 in progress | ADR-0027 stage 1 landed: 6 `Ok(vec![])`/`Ok(0)` stub defaults removed from `DbClient`, MockDbClient extended, 19 test doubles made explicit. Stage 2 (SQL moves into narrow stores + mock shrink): **reembed slice landed** (`ReembedStoreClient` owns `count/select_facts_needing_reembed`); **episode/archival slice landed** — `AppStoreClient::select_episodes_for_archival` + `EpisodeStoreClient::select_active_facts_by_episode` own their SQL, archival worker wired through stores, `MemoryService::episode_store()` added, 2 store unit tests, stale builder tests removed. Remaining: context → app → fact stores + behavior-test migration |
