...
| 6 | 🔶 partial | `find_intro_chain` relocated into `service/apps/graph.rs` as an inherent `MemoryService` method (ADR-0024 step 1) — core.rs is now construction + lifecycle + pipeline. The remaining mass migration (~30 test/eval call sites from `service.extract(...)`-style delegates to capability structs) is **blocked on a public-API decision**: `capabilities` is `pub(crate)` per ADR-0023, while `tests/*.rs` + `crates/eval-harness` are external crates — migrating them requires exposing `pub mod capabilities` (API expansion). Run the grilling loop before locking that shape. |
| 7 | ⏸ design-first | Plan calls this "design-it-twice territory — run the interface options through the grilling loop before locking the shape" (tier-fallback strategy objects, `RetrievalTier` relocation, 2,577-line `context.rs` test-block redistribution). No blind execution until the interface shape is agreed. |
