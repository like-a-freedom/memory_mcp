...
| 2 | 🔶 stage 2 in progress | ADR-0027 stage 1 landed: 6 `Ok(vec![])`/`Ok(0)` stub defaults removed from `DbClient`, MockDbClient extended, 19 test doubles made explicit. Stage 2 (SQL moves into narrow stores + mock shrink): **reembed**, **episode/archival**, **triple/entities**, **active-facts/batch**, **communities/edges**, and **communities-by-summary** slices landed — reembed/archival/triple/active-facts/batch/communities-by-member/relate-edge/communities-by-summary SQL owned or executed by stores; decay worker through `app_store()`; 2 integration-test `relate_edge` callers routed via `EpisodeStoreClient`; 3 behavior tests migrated to real in-memory SurrealDB + 3 community-summary doubles routed through `query` dispatch; `DbClient` now ≈ 12 methods (core 7 + retrieval/edges yet to move). Remaining: `select_facts_filtered`/`ann`/`entity_links`, `select_edges_*`/`select_edge_neighbors`, `select_entity_lookup`, `select_episodes_by_content` + their behavior-test migrations
