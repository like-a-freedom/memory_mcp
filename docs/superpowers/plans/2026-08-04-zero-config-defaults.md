...
- The existing NER default is `NerProviderKind::Anno`; this plan does not silently change it to regex or claim that NER has zero first-run cost. The NER backend registry (ADR-0029) remains; the default provider must stay `Anno` and the `BACKENDS` registry entry for `LocalGliner` must remain feature-gated in Task 10.
...
- Adding the public `init` CLI subcommand requires and is blocked on ADR-0030, which explicitly amends the existing ordinary-CLI freeze in ADR-0016, plus a live parser/surface test update.
...
- Do not promise a slim default binary before the ML feature-gating work is implemented and measured; Candle, `hf-hub`, and `tokenizers` are currently unconditional dependencies of the `memory_mcp` crate (see `crates/memory-mcp/Cargo.toml`).
- ADR-0028 ("Enable candle-accelerate by default") is already merged; Task 10's feature-gating must keep `accelerate`/`metal` wired only into the `ml` feature and must not regress that decision.
...
## Repository baseline (verified 2026-08-06)

Before beginning TDD, run `cargo check --workspace --all-targets --locked` on the clean implementation branch. As of 2026-08-06 the working tree compiles (`cargo check -p memory_mcp` passes) and the only uncommitted change is a modified `README.md` that predates this plan; do not overwrite unrelated user edits in that file — fold the Task 6/8 README corrections around them. Keep unrelated user changes untouched. Do not re-introduce the stale `data_dir_or_default()` expectation: the current helper in `crates/memory-mcp/src/config/helpers.rs` resolves `<exe-parent>/data/surrealdb`, and `crates/memory-mcp/src/config/surreal.rs:394-410` has a test asserting that legacy path; Task 1/2 replace both.
...
| `crates/memory-mcp/tests/agent_memory_lifecycle_release_gate.rs` | Extend the frozen ordinary CLI-subcommand snapshot with `init`; switch the three derived names from snake_case to kebab-case to match the live Clap parser. |
...
| `crates/memory-mcp/src/storage/helpers.rs` | Stage 1: apply the same trim/scheme normalization before the existing HTTP/HTTPS-to-WebSocket conversion. |
| `crates/memory-mcp/src/service/core/builder.rs` | Stage 1: emit `config.default_applied` / `config.legacy_data_dir_detected` provenance events through the existing startup logger. |
| `crates/memory-mcp/src/service/entity_extraction.rs` | Stage 1: retain the `create_entity_extractor_defaults_to_anno` regression test; Stage 7: feature-gate the `gliner` module, its registry entry, and its re-export. |
| `docs/adr/0030-zero-config-cli-init.md` | Record the explicit public CLI surface expansion and why `init` is output-only; amend the existing CLI freeze. |
...
| `scripts/measure_ttv.sh`, `scripts/test_measure_ttv.sh`, `scripts/measure_ttv_fixtures/` | Stage 6: measure clean-machine install-to-first-recall time for three personas. |
...
**Files:**
- Modify: `crates/memory-mcp/src/config/surreal.rs` (the `from_env()` body at lines ~79-131, the struct fields at ~25-53, and the test module)
- Create: `crates/memory-mcp/tests/zero_config_embedded.rs` for the real RocksDB root/root round-trip
- Modify: `crates/memory-mcp/src/service/core/builder.rs` to emit provenance events through the existing startup logger
...
In `crates/memory-mcp/src/service/entity_extraction.rs`, keep the exact test name `create_entity_extractor_defaults_to_anno` (currently at line ~367) and assert the default extractor reports `provider_name() == "anno"`; this test is part of the zero-config contract and must remain available after any backend-registry cleanup.
...
**Files:**
- Modify: `crates/memory-mcp/src/runner.rs` (`report_cli_error` at ~134-145 and `boxed_to_failure` at ~199-208)
...
### Task 4: Record the public CLI-surface decision in ADR-0030

**Files:**
- Create: `docs/adr/0030-zero-config-cli-init.md`
...
```bash
git add docs/adr/0030-zero-config-cli-init.md docs/adr/0016-agent-memory-lifecycle-integration.md CONTEXT.md docs/agent_integration/CONTRACT.md
...
- Modify: `crates/memory-mcp/src/cli.rs` (the `Command` enum at ~34-74)
- Modify: `crates/memory-mcp/src/runner.rs` (dispatch at ~47-129, `mode_label` at ~174-188)
...
**Interfaces:**
- Consumes: ADR-0030 and existing `clap::Args`/`clap::Subcommand` conventions.
...
Update the existing snapshot to use the actual Clap command spellings and append `init`. The current constant uses snake_case for three derived names (`assemble_context`, `lifecycle_capture`, `lifecycle_recall`); clap derives kebab-case names (`assemble-context`, `lifecycle-capture`, `lifecycle-recall`) from the same enum variants, so the constant must switch to kebab-case before the live test can pass:
...
Update the constant's doc comment to say that `init` is the one authorized output-only exception rather than claiming the pre-change command count.
...
Expected: both new tests compile but fail because `Command` does not yet expose `init`; the live test also protects the snapshot from future self-referential edits. The existing constant-only `public_surface_snapshot` is not sufficient because it does not inspect the live Clap command.
...
- Preserve any unrelated user edits already present in the local `README.md`; do not overwrite them wholesale.
...
- Modify: `crates/memory-mcp/src/storage/client.rs` (the private `DbEngine` enum and the `connect` selection)
...
**Files:**
- Modify: `.github/workflows/ci.yml` (the existing `build_binaries` release job around lines 215-291; do not create a duplicate workflow)
- Modify: `README.md`
...
**Interfaces:**
- Consumes: the completed Task 9 baseline evidence, the existing `EmbeddingProviderKind::Disabled` and `NerProviderKind::Anno` paths, and the ADR-0029 backend registry.
...
- The frozen ordinary CLI snapshot includes the authorized `init` command using live Clap kebab-case names.
...
- Deferred Candle feature-gating, including parent-module re-exports, the ADR-0029 backend registry, the ADR-0028 accelerate wiring, and eval-harness feature forwarding: Task 10, explicitly gated on the Task 9 baseline and retaining `default = []`.
...
## Corrections from the previous draft (2026-08-06 repository audit)

- The frozen test path is `crates/memory-mcp/tests/agent_memory_lifecycle_release_gate.rs`, not repository-root `tests/...`.
- The frozen CLI snapshot currently uses snake_case (`assemble_context`, `lifecycle_capture`, `lifecycle_recall`) for three derived subcommands, but the live Clap parser derives kebab-case names (`assemble-context`, `lifecycle-capture`, `lifecycle-recall`); Task 5 switches the snapshot to kebab-case and adds a live-parser comparison test so the two cannot drift again.
- The next available ADR is `0030`: the repository now contains ADRs through `0029` (`0027-finish-dbclient-capability-narrowing`, `0028-enable-candle-accelerate-by-default`, `0029-registry-of-pluggable-ner-backends`). All references to the new ADR use `0030` consistently (Global Constraints, file map, Task 2 Step 9, Task 4 heading/files/ADR text, Task 5 interface note, self-review).
- NER extraction was refactored into a registry-backed pluggable backend (ADR-0029): `crates/memory-mcp/src/service/entity_extraction.rs` now holds a `BACKENDS` registry with a `Kind::LocalGliner` entry at ~line 131, and the `create_entity_extractor_defaults_to_anno` test at ~line 367. Task 2 keeps that test; Task 10 gates the gliner module, its registry entry, and its re-export behind `ml` and replaces the registry `.expect(...)` with a descriptive `MemoryError::ConfigInvalid`.
- ADR-0028 ("Enable candle-accelerate by default") is merged; Task 10's feature wiring routes `accelerate`/`metal` through `ml` and must not regress the decision.
- The working tree baseline (2026-08-06) compiles and carries only a pre-existing modified `README.md`; the plan's baseline note reflects this and tells implementers to preserve unrelated user edits in that file.
- The `cli/commands/init.rs` module does not exist yet; Task 5 creates it and Task 6 replaces its stub.
- `scripts/` does not exist yet; Task 9 creates the three script/fixture paths.
