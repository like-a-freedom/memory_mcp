...
export MEMORY_HOOK_EVENT="stop"
export MEMORY_HOOK_DEFAULT_POLICY_TAGS="hook:stop,session_summary"
...
def build_content(payload: dict, raw_fallback: str, event_name: str) -> str:
    ...
    for key in ("session_id", "conversation_id", "status", "trigger", "hook_event_name"):
...
def main() -> int:
    event_name = os.environ.get("MEMORY_HOOK_EVENT", "stop")
    ...
    if os.environ.get("MEMORY_HOOK_VERBOSE", "") == "1":
        print(f"memory-stop-hook ingested summary into scope={scope}")
    return 0
