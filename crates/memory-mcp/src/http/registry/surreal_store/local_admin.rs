...
use crate::http::registry::models::{AccountStatus, ApiKeyMeta, ApiKeyStatus, TenantStatus};
...
use super::{SurrealRegistryStore, datetime_value, record_id_value, status_from_row};
...
impl LocalAdminStore for SurrealRegistryStore {
    ...
    async fn reserve_attempt(&self, input: AttemptInput) -> LocalResult<AttemptDecision> {
        ...
        let rows = loop {
            match self
                .db
                .as_dyn()
                .query_json_at(sql, Some(vars.clone()), 11)
                .await
            {
                Ok(rows) => break rows,
    ...
    /// Result index 0 is the `{ items: [...] }` envelope; the
    /// `creating_admin_id` filter means a local admin only ever sees
    /// sidecar-backed clients it created.
    async fn list_clients(
        ...
        let admin_id = fence.admin_id.clone();
        ...
        let sql = format!(
            ...
            WHERE creating_admin_id = $admin_id AND account_id > $cursor
    ...
    async fn client(&self, fence: &AdminFence, account_id: &str) -> LocalResult<ClientView> {
        ...
        let sql = format!(
            ...
            SELECT account_id, display_name, account_status, tenant_status, plan_version, schema_version, version,
                   {CLIENT_LIVE_PROJECTION}
            FROM local_admin_client
            WHERE account_id = $account_id AND creating_admin_id = $admin_id
            LIMIT 1;
        "
        );
        ...
        let rows = self
            ...
            .query_json(
                &sql,
                Some(json!({"account_id": account_id, "admin_id": admin_id})),
            )
            .await
            .map_err(infra)?;
    ...
    /// Ownership is checked against the sidecar first (result index 0 of
    /// the guard read); the page is then read with a stable `key_id`
    /// cursor.
    async fn list_client_keys(
        ...
        let owner = self
            ...
            .as_dyn()
            .query_json(
                "SELECT account_id FROM local_admin_client WHERE account_id = $account_id AND creating_admin_id = $admin_id LIMIT 1;",
                Some(json!({"account_id": account_id, "admin_id": admin_id})),
            )
            .await
            .map_err(infra)?;
        ...
        let sql = "
            RETURN { items: (SELECT key_id, name, created_at, expires_at, revoked
            FROM local_admin_client_key
            WHERE account_id = $account_id AND key_id > $cursor
            ORDER BY key_id ASC
            LIMIT $limit) };
        ";
        ...
        let items = decode_page(rows.first(), decode_key_meta)?;
        let next_cursor = if items.len() == usize::from(limit) {
    ...
    /// bearer authorization (`RegistryStore::find_api_key`), and
    /// `local_admin_client_key` is the local ownership/issuance sidecar
    /// the admin surface lists and scopes revocation by. Because both
    /// rows commit together (and revocation updates both together), the
    /// sidecar cannot drift from the authoritative authorization row.
    async fn insert_client_key(
        ...
        let sql = String::from("BEGIN TRANSACTION;")
            + MUTATION_GUARD
            + r#"
            LET $client = (SELECT plan_version FROM local_admin_client WHERE account_id = $account_id AND creating_admin_id = $admin_id LIMIT 1);
            ...
            SELECT key_id, name, created_at, expires_at, revoked FROM local_admin_client_key WHERE key_id = $target LIMIT 1;
            COMMIT TRANSACTION;
        "#;
    ...
    async fn revoke_client_key(
        ...
        let sql = String::from("BEGIN TRANSACTION;")
            + MUTATION_GUARD
            + r#"
            LET $client = (SELECT account_id FROM local_admin_client WHERE account_id = $account_id AND creating_admin_id = $admin_id LIMIT 1);
    ...
    async fn set_client_state(
        ...
        let sql = String::from("BEGIN TRANSACTION;")
            + MUTATION_GUARD
            + r#"
            LET $client = (SELECT version, suspended_from_ready FROM local_admin_client WHERE account_id = $account_id AND creating_admin_id = $admin_id LIMIT 1);
            IF array::len($client) = 0 { THROW 'not_found'; };
            LET $account = (SELECT tenant_id, status FROM account WHERE id = type::record('account', $account_id) LIMIT 1);
            IF array::len($account) = 0 { THROW 'state_conflict'; };
            LET $tenant = (SELECT status FROM tenant WHERE id = type::record('tenant', $account[0].tenant_id) LIMIT 1);
...
fn decode_key_meta(row: &Value) -> LocalResult<ApiKeyMeta> {
    let revoked = row
        .get("revoked")
        .and_then(Value::as_bool)
        .ok_or_else(|| LocalAdminError::InvalidInput("missing field: revoked".into()))?;
    Ok(ApiKeyMeta {
        id: require_str(row, "key_id")?,
