...
What to do when a tool call returns an error or an unexpected shape — MCP or CLI. The parent skill's `SOP exit gate` requires failures be reported, not papered over.

## Common errors

| Symptom | Likely cause | Recovery |
|---|---|---|
| `Invalid t_ref` / `Invalid t_invalid` | Non-ISO 8601 or non-UTC string | Re-issue with `YYYY-MM-DDTHH:MM:SSZ`. The source's own time, not now. |
...
| `Unknown app` (open_app) | App name not in the closed set | Use one of `inspector`, `diff`, `ingestion_review`, `lifecycle`, `graph`. Do not invent app names. |
| `Unknown action` (app_command) | Action not in the closed set for that session's app | Read the action list from the `app_command` tool description and pick one. Do not invent actions. |
| `MCP apps are disabled` | `mcp-apps` feature not enabled at build | This is a server-side configuration fact, not a retryable error. Surface the configuration state to the operator; do not switch to canonical tools in a way that loses the user's intent. |
...
- **No writes in a read path, ever.** A failed `assemble_context` is not a license to call `ingest` to "seed" the answer. If a fact must be captured, end the retrieval and run the capture SOP.
- **No scope-widening on a hunch.** If `assemble_context` at `team` returns nothing, the absence is informative; widening to `org` should be justified by evidence that the fact lives there, not by the desire to fill a void.
