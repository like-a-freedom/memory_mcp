...
|---|---|---|
| `ingest` | `source_type`, `source_id`, `content`, `t_ref`; pass `scope` explicitly although it defaults | created or existing `episode_id` |
...
| `assemble_context` | `query`, `scope`; optional project, fact types, `as_of`, budget, view/window fields | ranked context items |
...
the live schema for optional fields and enums rather than inventing them.
...
| `lifecycle` | scope-wide maintenance |
